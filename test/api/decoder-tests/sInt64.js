...
function getWaypointManager(data, callback) {
    data.client.send(data.client.services.spaceCenter.getWaypointManager(), function(
        err,
        response
    ) {
        if (err) {
...
function getWaypoints(data, callback) {
    data.client.send(
        data.client.services.spaceCenter.waypointManagerGetWaypoints(data.waypointManager),
...
function waypointGetContractId(data, callback) {
    data.client.send(
        data.client.services.spaceCenter.waypointGetContractId(data.waypoints[0]),
        function(err, response) {
            if (err) {
                return callback(err);
            }
