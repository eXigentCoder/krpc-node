...
function addDirectInvoker(service, client) {
    ...
    Object.keys(service).forEach(key => {
        ...
        wrappedService['_' + key] = async function() {
            const inokerOptionsIndex = invokerOptionsIndex(arguments);
            if (inokerOptionsIndex >= 0) {
                const options = arguments[inokerOptionsIndex];
                Array.prototype.splice.call(arguments, inokerOptionsIndex, 1);
...
function invokerOptionsIndex(args) {
    if (!args || args.length < 1) {
        return -1;
    }
    let foundIndex = -1;
    Array.prototype.forEach.call(args, doWork);
    return foundIndex;

    function doWork(arg, index) {
