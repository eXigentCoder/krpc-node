 ...
 *
 */
module.exports = async function createClient(options, callback) {
    options = options || {};
...
async function buildClient(options) {
    ...
    let client = {
        ...
        close: async function close(callback) {
            if (client.stream && client.stream.socket) {
                await client.stream.socket.close(1000);
            }
            await client.rpc.socket.close(1000);
            if (callback) {
                callback();
            }
        }
...
function onMessage(client, event) {
    //TODO RK The callback stack is going to get messed up if there is an error here, need to update this code.
    let response;
    try {
