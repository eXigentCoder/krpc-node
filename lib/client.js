...
function buildClient(options, callback) {
    let client = {
        ...
        streams: {},
        streamState: {},
        addStream: function addStream(call, propertyName, sendCallback) {
            _addStream(client, call, propertyName, sendCallback);
        }
    };
...
function onStreamMessage(client, event) {
    ...
    streamUpdate.results.forEach(function (update) {
        ...
        client.streamState[stream.propertyName] = stream.decode(update.result.value);
    });
    return client.stream.emitter.emit('message', client.streamState, streamUpdate, event);
}
...
function _addStream(client, procedure, propertyName, callback) {
    if (!client.stream) {
