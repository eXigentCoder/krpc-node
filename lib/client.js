...
function onMessage(options, event) {
    ...
    try {
        ...
        response = proto.root().Response.decode(buff);
    }
    catch (error) {
        let parsedMessage;
        try {
    ...
    response.results = response.results.map(function (result) {
        ...
        result.value = decoder(toBuffer(result.value));
...
function onStreamMessage(options, event) {
    let streamUpdate;
    try {
        streamUpdate = proto.root().StreamUpdate.decode(event.data);
    }
    catch (error) {
        let parsedMessage;
        try {
...
function sendRequest(options, calls) {
    ...
    let req = proto.root().Request.create({calls: procedureCalls});
    let buff = proto.root().Request.encode(req).finish();
