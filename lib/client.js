...
function onMessage(client, event) {
    ...
    response.results = response.results.map(function (result) {
        ...
        } else if (decoder.isAddStreamCall) {
            result.value = decoder.decode.decode(result.value);
            client.stream.subscribed[result.value.id.toString()] = {
                name: "",
                decode: null
            };
        } else {
...
function sendRequest(client, calls, callback) {
    ...
    calls.forEach(function (call) {
        ...
        if (call.call.service === "KRPC" && call.call.procedure === "AddStream") {
            let newDecode = {
                decode: call.decode,
                isAddStreamCall: true
            };
            client.decodeStack.splice(0, 0, newDecode);
        } else {
