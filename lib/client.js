...
function decodeCollection(decoder, data) {
    ...
    //         let subDecoder = decoder.subTypes[i];
    //         if (!subDecoder) {
    //             throw new Error(util.format('No sub-decoder', i, decoder));
    //         }
    //         subData.items[i] = decode(subDecoder, subData.items[i]);
    //     }
    // }
...
function onStreamMessage(client, event) {
    ...
    streamUpdate.results.forEach(function (update) {
        ...
        let decodedValue;
        if (stream.decode instanceof ProtoBuf.Type) {
            decodedValue = stream.decode.decode(update.result.value);
        } else {
            decodedValue = stream.decode(update.result.value);
        }
...
function _addStream(client, procedure, propertyPath, callback) {
    if (!client.stream) {
        return callback(new Error("Before you call `client.addStream` you must first connect to the stream server by calling `client.connectToStreamServer`."));
    }
    if (_.isArray(procedure)) {
        return callback(new Error("You can only pass one procedure call to `client.addStream` at a time."));
    }
    if (!procedure.call) {
...
function _removeStream(client, propertyPath, callback) {
    ...
    if (typeof existingStreamValue === "undefined") {
        return callback(new Error(util.format('client.streamState.%s was not set and cannot be removed.', propertyPath)));
    }
    let existingStream = _.get(client.streamLink, propertyPath);
    if (!existingStream) {
        return callback(new Error(util.format('client.streamLink.%s was not set and cannot be removed.', propertyPath)));
    }
