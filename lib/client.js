...
module.exports = function Client(options) {
    ...
    var socket = new WebSocket(options.protocol + '://' + options.host + ':' + options.port, options.wsProtocols, options.wsOptions);
...
function onMessage(client, event) {
    ...
    try {
        ...
        var errorMessage = util.format(error.message, os.EOL, parsedMessage);
        client.emitter.emit('error', new Error(errorMessage));
    }
