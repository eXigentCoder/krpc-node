...
module.exports.addStream = function addStream(procedureCall) {
    let value = proto.ProcedureCall.encode(procedureCall).finish();
    let argument = proto.Argument.create({position: 0, value: value.buffer});
    let newCall = proto.ProcedureCall.create({service: 'KRPC', procedure: 'AddStream', arguments: [argument]});
    return {
        call: newCall,
        decode: proto.Stream
    };
